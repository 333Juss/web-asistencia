...
export class AuthService {
    ...
    private redirectByRole(rol: string): void {
        ...
        switch (rol) {
            case 'ADMIN':
            case 'RRHH':
                ruta = '/admin';  // ← Simplificado a /admin
                break;
            case 'EMPLEADO':
                ruta = '/employee';  // ← Simplificado a /employee
                break;
            case 'SUPERVISOR':
                ruta = '/admin';  // ← SUPERVISOR también va a admin
                break;
            default:
                ruta = '/';
        }
    ...
    changePassword(oldPassword: string, newPassword: string): Observable<ApiResponse<any>> {
        return this.http.post<ApiResponse<any>>(`${'http://localhost:8080'}/api/usuarios/change-password`, {
            oldPassword,
            newPassword
        }).pipe(
            tap(response => {
