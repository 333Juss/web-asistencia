...
export class MarcarAsistenciaComponent implements OnInit {
    ...
    // Configuración del umbral de reconocimiento facial (80% = 0.80)
    private readonly UMBRAL_CONFIANZA = 0.80;
        ...
        this.geolocationService.getCurrentPosition().subscribe({
            next: (location: UserLocation) => {
                ...
                if (this.dentroDeRadio) {
                    ...
                    this.notificationService.error(`Estás fuera del perímetro de la sede (${this.distancia.toFixed(0)}m). Acércate más.`);
                    this.currentStep = 'idle';
                    this.tipoMarcacion = undefined;
                }
            },
        ...
        this.reconocimientoService.verificarRostro({
            ...
            next: (response) => {
                if (response.success && response.data) {
                    ...
                    if (response.data.coincide && confianza >= this.UMBRAL_CONFIANZA) {
                        ...
                        if (confianza < this.UMBRAL_CONFIANZA) {
                            this.notificationService.error(
                                `Reconocimiento facial insuficiente (${porcentajeConfianza}%). Se requiere al menos 80%`
                            );
                        } else {
                    ...
                    // Mostrar mensaje del backend (incluyendo DEBUG ERROR)
                    const msg = response.message || 'No se pudo verificar tu rostro';
                    this.notificationService.error(msg);
                    alert('DEBUG BACKEND MESSAGE: ' + msg);
                }
            },
            error: () => {
                this.procesando = false;
    ...
    private marcarEntrada(imagenBase64: string, confianza: number): void {
        ...
        this.asistenciaService.marcarEntrada(dto).subscribe({
            ...
            error: (err) => {
                ...
                this.currentStep = 'idle';
                this.tipoMarcacion = undefined;
                const errorMessage = err.error?.message || err.message || JSON.stringify(err.error) || 'Error desconocido';
                alert('DEBUG ERROR: ' + errorMessage); // Forzando alerta para ver el error
