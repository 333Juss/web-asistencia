...
export class AsistenciaService {
  ...
  listarFaltas(colaboradorId: number): Observable<any[]> {
    return this.http
      .get<{ success: boolean; data: any[] }>(`${this.apiUrl}/inasistencias/${colaboradorId}`)
      .pipe(map(res => res.data));
  }

  listarInasistenciasConJustificacion(colaboradorId: number): Observable<any[]> {
    return this.http
      .get<{ success: boolean; data: any[] }>(
        `${this.apiUrl}/inasistencias/${colaboradorId}/con-justificacion`
      )
      .pipe(map(res => res.data));
  }
